...

{
    ...
    while(sf("%d%lf%lf%lf%lf",&n,&gx,&gy,&dx,&dy)==5)
    {
        ...
        for(int i=0; i<n; i++)
        {
            ...
            if(ck)
            {
                double dis1 = distanc(gx,gy,x,y);
                double dis2 = distanc(dx,dy,x,y);
                if(dis2/2>=dis1)
                {
                    rx=x;
                    ry=y;
        ...
        if(!ck)
            pf("The gopher can escape through the hole at (%0.3f,%0.3f).\n",rx,ry);
        else
            pf("The gopher cannot escape.\n");
    }
    return 0;
}
